...
def get_win_counter():
    with open("win_counter.txt", "r") as file:
        return int(file.read())


def write_win_counter(number: int):
    with open("win_counter.txt", "w") as file:
        return file.write(str(number))


def add_win_counter():
    counter = get_win_counter()
    counter = counter + 1
    write_win_counter(counter)
